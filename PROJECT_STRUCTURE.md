...
│
├── news-parser/             # Python news fetching service (planned)
│   ├── services/            # Service layer
│   │   ├── rss_scraper.py  # RSS feed fetching and parsing
│   │   ├── scraper_service.py  # Generic HTTP scraper
│   │   └── parser_service.py   # Generic HTML parser
│   │
│   ├── database/            # Database layer
│   │   └── db_manager.py   # PostgreSQL operations for news articles
│   │
│   ├── utils/               # Utilities
...
│   │
│   ├── config.py            # News parser configuration
│   ├── main.py              # Service entry point
│   ├── test_feeds.py        # RSS feed validation script
│   ├── requirements.txt     # Python dependencies
│   ├── .env.example         # Example environment file
│   ├── README.md            # News parser documentation
...
- **News Parser**: `news-parser/main.py` - News fetching service (planned)
...
- `news_articles` - News articles from RSS feeds (migration 000015)
- `article_stock_mentions` - Article-to-ticker associations
...
### 4. News Scraping Flow (Planned)
```
RSS Feeds (CafeF, VnExpress, Vietstock, NDH)
    ↓ Scraped by
News Parser (news-parser/main.py)
    ↓ Every 30 minutes
Database (news_articles table)
    ↓ Future integration
Sentiment Analysis
    ↓ Event detection
Signal Generation (enhanced with news signals)
```
...
### Python Services
**ml-service/** - All ML services are batch/scheduled jobs, not real-time:
...
**news-parser/** - News fetching service (in development):
- **services/**: News source integrations and RSS parsers
- **database/**: PostgreSQL operations for news articles
- **utils/**: Logging and utilities
- Scrapes 5 Vietnamese financial news RSS feeds every 30 minutes
- Stores articles in `news_articles` table for future analysis
- Future: Sentiment analysis, event-driven trading signals, ticker extraction
...
- News fetching? → `news-parser/` (planned)
